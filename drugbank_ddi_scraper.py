...
class DrugBankDDIScraper:
    ...
    def __init__(self, headless=False, delay_between_requests=2.5, debug=False):
        """
        ...
        Args:
            ...
            debug: Enable debug mode (screenshots, detailed logging)
        """
        ...
        self.context = None
        self.page = None
        self.debug_counter = 0  # For numbering debug screenshots
    ...
    async def initialize(self):
        ...
        # For demo mode: position browser on RIGHT half of screen
        if self.debug:
            browser_args.extend([
                '--window-position=960,0',   # Right half (x=960px from left edge)
                '--window-size=960,1080'      # Half screen width, full height
            ])
            print("🖥️  Browser positioned on RIGHT half - keep terminal on LEFT")
        ...
        # Adjust viewport for debug mode (half screen)
        viewport_width = 900 if self.debug else 1920  # Slightly smaller for half screen
        viewport_height = 1000 if self.debug else 1080
    ...
    async def check_interaction(self, drug1, drug2):
        ...
        try:
            ...
            if self.debug:
                print(f"\n{'='*80}")
                print(f"🌐 Navigating to DrugBank DDI Checker...")
            logging.debug(f"Navigating to {url}")
            await self.page.goto(url, wait_until='load', timeout=30000)

            # Wait for page to fully load
            await self.page.wait_for_timeout(2000)
            await self._debug_snapshot("initial_page")

            # Add first drug
            if self.debug:
                print(f"\n💊 Adding first drug: {drug1}")
            logging.debug(f"Adding first drug: {drug1}")
            await self._add_drug(drug1)
            await self._debug_snapshot(f"after_drug1_{drug1}")

            # Random delay between adding drugs (human-like)
            delay_ms = random.randint(1500, 2500)
            if self.debug:
                print(f"⏱️  Waiting {delay_ms/1000:.1f}s between drugs (human-like behavior)...")
            await self.page.wait_for_timeout(delay_ms)

            # Add second drug
            if self.debug:
                print(f"\n💊 Adding second drug: {drug2}")
            logging.debug(f"Adding second drug: {drug2}")
            await self._add_drug(drug2)
            await self._debug_snapshot(f"after_drug2_{drug2}")
            ...
            if self.debug:
                print(f"\n🔍 Clicking 'Check Interactions' button...")
            logging.info("Clicking 'Check Interactions' button...")
            try:
                await self._click_check_button()
                await self._debug_snapshot("after_click_button")
            ...
            await self.page.wait_for_timeout(2000)
            await self._debug_snapshot("results_loaded")

            # Extract severity and text
            if self.debug:
                print(f"\n📊 Extracting severity and interaction details...")
            severity, interaction_text = await self._extract_severity_and_text(drug1, drug2)

            if self.debug:
                print(f"✅ Severity: {severity}")
                print(f"📝 Description: {interaction_text[:100]}..." if len(interaction_text) > 100 else f"📝 Description: {interaction_text}")
    ...
    async def _debug_snapshot(self, step_name):
        """
        Take debug snapshot (screenshot + HTML) if debug mode is enabled

        Args:
            step_name: Name of the current step (e.g., 'after_first_drug')
        """
        if not self.debug:
            return
        ...
        try:
            ...
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            prefix = f"debug_{self.debug_counter:02d}_{step_name}_{timestamp}"
            ...
            logging.warning(f"Could not save debug snapshot: {e}")
...
async def process_drug_pairs(input_file, output_file=None, checkpoint_file="ddi_checkpoint_drugbank.csv", start_row=0, end_row=None, debug=False):
    """
    ...
    # Add columns if they don't exist (in order)
    ...
    # Add Final_Severity column right after DrugBank_Text
    final_severity_col = 'Final_Severity'
    if final_severity_col not in df.columns:
        # Insert Final_Severity after DrugBank_Text
        text_col_idx = df.columns.get_loc(text_col)
        df.insert(text_col_idx + 1, final_severity_col, pd.Series(dtype='object'))
        logging.info(f"Added {final_severity_col} column (after {text_col})")
    ...
    if debug:
        ...
        print("  - Browser will be visible (RIGHT half of screen)")
        print("  - Screenshots will be saved at each step")
        print("  - Detailed logging enabled")
        print("  - Clearing old checkpoints and logs...")
        print("="*80 + "\n")

        # Clear old checkpoint file in debug mode
        if Path(checkpoint_file).exists():
            Path(checkpoint_file).unlink()
            print(f"✓ Cleared checkpoint: {checkpoint_file}")

        # Clear old debug screenshots/HTML
        import glob
        old_debug_files = glob.glob("debug_*.png") + glob.glob("debug_*.html")
        if old_debug_files:
            for f in old_debug_files:
                try:
                    Path(f).unlink()
                except:
                    pass
            print(f"✓ Cleared {len(old_debug_files)} old debug files\n")

    scraper = DrugBankDDIScraper(headless=False, delay_between_requests=2.5, debug=debug)
    ...
    try:
        ...
        for idx in range(start_row, end_row):
            ...
            # Populate Final_Severity based on DrugBank_Severity
            # If no severity extracted or error, default to NoInteraction
            scraped_severity = result['DrugBank_Severity']
            if scraped_severity in ['Error', 'Timeout', None, '', 'TBD'] or pd.isna(scraped_severity):
                df.at[idx, final_severity_col] = 'NoInteraction'
            else:
                df.at[idx, final_severity_col] = scraped_severity
...
async def main():
    ...
    parser.add_argument('--debug', action='store_true',
                       help='Enable debug mode: visible browser, screenshots, detailed logging')
    ...
    await process_drug_pairs(args.input_file, args.output, args.checkpoint, args.start, args.end, args.debug)
